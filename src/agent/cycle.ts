  ...
  try {
    ...
    await journal?.recordArtifact("pass-edge-audit", {
      ...
      baseline: {
        runId,
        cycleId,
        candidate: "deployment-specific-baseline-not-embedded",
        result:
          "Deployment-specific historical comparison data is intentionally not embedded in the public engine.",
      },
      final: passEdgeAudit,
    ...
    const validationExecution = await withStageTimeout(
      ...
      async (signal) => {
        ...
        const firstAccepted = validation.accepted[0];
        const secondAccepted = validation.accepted[1];
        await journal?.recordArtifact("concentration-experiment", {
          schemaVersion: 1,
          experimentId: "conditional-concentration-v1",
          hypothesis:
            "Record how configured sizing ceilings constrain independently validated targets without changing evidence, settlement, spread, depth, or uncertainty safeguards.",
          baseline: {
            maximumPositionCostBasisFraction:
            ...
            runId,
            cycleId,
            observedCycleSpendCapacity: cycleSpendCapacity.toFixed(),
            firstAcceptedMaximumSpend:
              firstAccepted?.maximumExecutionSpend.toFixed() ?? "0",
            secondTargetRequestedRisk:
              secondAccepted?.proposal.maximumRiskUsd.toFixed() ?? "0",
            secondAcceptedMaximumSpend:
              secondAccepted?.maximumExecutionSpend.toFixed() ?? "0",
            result:
              "No deployment-specific historical comparison is embedded; values describe the current validated cycle.",
          },
          treatment: {
            maximumPositionCostBasisFraction:
              dependencies.config.risk.maximumPositionCostBasisFraction.toFixed(),
