import { nearTouchBuyNotional } from "../execution/depth.js";
import { FileExecutionHealth } from "../execution/execution-health.js";
import {
...
function materializeTargetDecision(
  ...
  const reconciliation = reconcilePortfolioTargets({
    ...
    snapshot,
    riskEquity: valuation.riskEquity,
    targetRoundingToleranceUsd:
      strategy.reconciliationTolerance?.(valuation.riskEquity) ??
      new Decimal(0),
  });
  ...
  try {
    ...
    const discovery = await withStageTimeout(
      ...
      async (signal) => {
        ...
        const opportunityBoard =
          await strategy.selection.buildEnrichedOpportunityBoard(
            catalog,
            ...
            async (marketSlug, enrichmentSignal) => {
              ...
              if (
                ...
                try {
                  ...
                  enrichmentSignal?.throwIfAborted();
                  yesNearTouchBuyNotionalUsd = nearTouchBuyNotional(
                    book,
                    "YES",
                    strategy.selection.depthPriceBand ?? new Decimal(0),
                  ).toNumber();
                  noNearTouchBuyNotionalUsd = nearTouchBuyNotional(
                    book,
                    "NO",
                    strategy.selection.depthPriceBand ?? new Decimal(0),
                  ).toNumber();
              ...
              return {
                ...
                ...(noNearTouchBuyNotionalUsd === undefined
                  ? {}
                  : { noNearTouchBuyNotionalUsd }),
        ...
        return {
          ...
          marketSelectionExperiment:
            strategy.selection.experimentDefinition === undefined
              ? undefined
              : replayMarketSelectionExperiment(
                  marketSelectionSnapshot,
                  strategy.selection.experimentDefinition,
                  strategy.selection.buildOpportunityBoard,
                ),
        };
      },
    ...
    const requiredPassedPriorityMarketSlugs = new Set(
      strategy.selection
        ...
        .filter((slug) => opportunityBoard.some((item) => item.slug === slug)),
    );
    ...
    const resolveMarketFamilyDetails = async (
      ...
      const bookSnapshots: readonly {
        ...
        family.members.map(async (member) => {
          if (!(
            strategy.selection.shouldFetchFamilyBook?.(member.market) ?? false
          )) {
            return {};
          }
          try {
    ...
    const allowWebSearch =
      strategy.selection.allowWebSearch?.(
        opportunityBoard,
        modelSnapshot.positions.length,
      ) ?? true;
    const liveDecisionLimits = allowWebSearch
    ...
    const researchTools = new DecisionResearchTools({
      ...(strategy.selection.shouldEnforceRequiredResearch === undefined
        ? {}
        : {
            requiredResearchGate:
              strategy.selection.shouldEnforceRequiredResearch,
          }),
      ...(strategy.forecast === undefined
        ? {}
        : { forecastPolicy: strategy.forecast }),
      marketDetails: [
    ...
    const validateDecisionProposals = async (
      ...
      const freshLive = await refreshForecasts(
        proposals,
      ...
      const [evidence, baseCoverage, freshTargetLive, passAudit] =
        await Promise.all([
          ...
          auditNoPositiveEdgePasses({
            ...
            minimumMaterialEdge: strategy.passAuditMinimumEdge,
            signal,
          }),
        ]);
      ...
      for (const target of candidateDecision.portfolioTargets) {
        ...
        if (freshProbability === undefined) {
          liveCoverageIssues.push({
            ...
            message: `${target.marketSlug} requires a fresh forecast source state, but the resolver did not return a usable current probability`,
          });
          continue;
        }
        if (
          target.estimatedProbability
            .minus(freshProbability)
            .abs()
            .gt(strategy.forecast?.forecastTolerance ?? 0)
        ) {
          liveCoverageIssues.push({
    ...
    if (unresolvedPassEdgeIssues.length > 0) {
      const unresolvedMarketSlugs = [
        ...new Set(unresolvedPassEdgeIssues.map((issue) => issue.marketSlug)),
      ];
      warnings.push(
    ...
    await journal?.recordArtifact("pass-edge-audit", {
      schemaVersion: 1,
      treatment: {
        minimumMaterialNetEdge: `strictly greater than max(market price tick, ${strategy.passAuditMinimumEdge.toFixed()}) per contract`,
    ...
    const validationExecution = await withStageTimeout(
      ...
      async (signal) => {
        ...
        await journal?.recordArtifact("concentration-experiment", {
          ...
          limitation:
            "Sizing ceilings do not manufacture opportunities and never bypass forecasting, evidence, settlement, book, or positive-edge validation.",
        });
