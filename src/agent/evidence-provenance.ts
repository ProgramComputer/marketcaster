...
async function fetchSetkaLiveMatchFeed(
  pageUrl: URL,
  ...
  if (
    pageUrl.hostname.toLowerCase() !== "tabletennis.setkacup.com" ||
    pageUrl.pathname.toLowerCase().startsWith("/api/")
  ) {
    return undefined;
  }
  const feedUrl = await assertPublicEvidenceUrl(
    new URL("/api/Matches/widget/en", pageUrl).toString(),
    lookupImplementation,
  );
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const attemptTimeout = AbortSignal.timeout(10_000);
    const signal =
      callerSignal === undefined
        ? attemptTimeout
        : AbortSignal.any([callerSignal, attemptTimeout]);
    try {
      ...
      if (!/^application\/json(?:;|$)/u.test(contentType)) continue;
      const source = await readBoundedBody(response, 262_144);
      const feed = setkaMatchFeedText(source);
...
export async function fetchEvidencePage(
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    ...
    try {
      const setkaFeed = await fetchSetkaLiveMatchFeed(
        current,
        fetchImplementation,
        options.lookupImplementation ?? lookup,
        options.signal,
      );
      if (setkaFeed !== undefined) {
        text = `${text}\nOfficial Setka Cup live and recent-results feed:\n${setkaFeed}`;
      }
    } catch (error) {
      if (options.signal?.aborted === true) throw error;
      // The ordinary page remains valid evidence when optional first-party
