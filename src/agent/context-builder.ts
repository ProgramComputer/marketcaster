...
function buildCriticalLearning(
  ...
  const outcomes = [
    ...performance.settlements.map((outcome) => ({
      marketSlug: outcome.marketSlug,
      realizedPnl: outcome.realizedPnl,
    })),
    ...performance.closedTrades.map((outcome) => ({
      marketSlug: outcome.marketSlug,
      realizedPnl: outcome.realizedPnl,
    })),
  ];
  const profitableMarketSlugs = outcomes
    .filter((outcome) => outcome.realizedPnl.gt(0))
    .map((outcome) => outcome.marketSlug);
  const losingMarketSlugs = outcomes
    .filter((outcome) => outcome.realizedPnl.lt(0))
    .map((outcome) => outcome.marketSlug);
  const reminders = [
    "Use only the current exchange snapshot and fresh research as evidence; historical summaries and notes are advisory.",
    "Persistent plans are follow-up hints, not research mandates: independently scan expiring, trending or high-volume, and category-diverse non-held markets; ignore any legacy categorical ban such as avoiding sports.",
  ];
  if (previousCycle === undefined) {
    reminders.push(
      "No compatible previous-cycle advisory was available; do not invent a prior thesis or probability.",
    );
  } else {
    reminders.push(
      `Reconcile this cycle with prior status ${previousCycle.source.status} at ${previousCycle.source.completedAt}.`,
    );
    const priorEstimates =
      previousCycle.decision.portfolioTargets !== undefined &&
      previousCycle.decision.portfolioTargets.length > 0
        ? previousCycle.decision.portfolioTargets
        : previousCycle.decision.proposals;
    for (const proposal of priorEstimates) {
      reminders.push(
        `For ${proposal.marketSlug}, compare fresh evidence with the prior ${proposal.estimatedProbability} ${proposal.confidence} probability before changing the thesis.`,
      );
    }
    if (previousCycle.performance.rejectedProposals.length > 0) {
      reminders.push(
        `Do not repeat prior rejected proposals without resolving: ${previousCycle.performance.rejectedProposals
          .map((proposal) => `${proposal.marketSlug}:${proposal.code}`)
          .join(", ")}.`,
      );
    }
    const blockedTargets = (
      previousCycle.performance.targetReconciliations ?? []
    ).filter((reconciliation) => reconciliation.kind === "BLOCKED");
    if (blockedTargets.length > 0) {
      reminders.push(
        `Resolve prior blocked targets before repeating them: ${blockedTargets
          .map(
            (reconciliation) =>
              `${reconciliation.marketSlug}:${reconciliation.reason}`,
          )
          .join(", ")}.`,
      );
    }
  ...
  return {
    ...
    winningPatternAssessment:
      profitableMarketSlugs.length === 0
        ? "No realized profitable outcomes are available to support a repeatable winning pattern."
        : `Review profitable outcomes for repeatable, evidence-backed features without assuming causation: ${profitableMarketSlugs.join(", ")}.`,
    losingPatternAssessment:
      losingMarketSlugs.length === 0
        ? "No realized losing outcomes are available to establish a losing pattern."
        : `Before similar exposure, identify what was miscalibrated in these losing outcomes: ${losingMarketSlugs.join(", ")}.`,
    positionManagementReminders: reminders,
  };
}
...
export function buildAgentContext(input: BuildAgentContextInput): AgentContext {
  ...
  return {
    ...
    criticalLearning: buildCriticalLearning(performance, input.previousCycle),
    markets: {
