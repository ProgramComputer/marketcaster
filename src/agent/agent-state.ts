...
export class FileAgentState implements AgentState {
  ...
  private async readSnapshot(): Promise<SnapshotRead> {
    ...
    // Version 2 marks the portfolio-strategy reset. Preserve evidence-based
    // beliefs from v1, but discard old planning directives authored for the
    // former macro-first, low-concentration strategy.
    const migrated =
  ...
  private context(snapshot: AgentStateSnapshot): AgentStateContext {
    const beliefs = this.sortedBeliefs(snapshot).slice(
      0,
      this.maximumContextBeliefs,
    );
    return {
      mode: "PERSISTENT",
      beliefs,
  ...
  private newPlan(content: string, marketSlugs: readonly string[]): AgentPlan {
  ...
  public manage(
    ...
    const result = this.mutationQueue.then(async () => {
      ...
      if (parsed.action === "ADD_BELIEF") {
        ...
        snapshot = {
          ...read.snapshot,
          nextCyclePlan: this.newPlan(parsed.content, parsed.marketSlugs),
        };
      } else if (parsed.action === "SET_LONG_TERM_PLAN") {
        snapshot = {
          ...read.snapshot,
          longTermPlan: this.newPlan(parsed.content, parsed.marketSlugs),
        };
      } else {
