...
const ACTIVE_ORDER_STATES: ReadonlySet<OrderState> = new Set([
  "NEW",
  "OPEN",
  "INFLIGHT",
  "PARTIALLY_FILLED",
...
export function mapPosition(
  ...
  return {
    ...
    lifecycleState: value.expired ? "EXPIRED" : "UNKNOWN",
...
export function mapOrderExecutionPolicy(value: string): OrderExecutionPolicy {
  switch (value) {
    case "TIME_IN_FORCE_IMMEDIATE_OR_CANCEL":
      return "IOC";
    case "TIME_IN_FORCE_GOOD_TILL_DATE":
    case "TIME_IN_FORCE_GOOD_TILL_TIME":
      return "GTD";
...
export function mapOrder(
  ...
  const state = mapOrderState(value.state);
  const reportedRemainingQuantity = firstDefined(
    value.leavesQuantityDecimal,
    value.leavesQuantity,
  );
  const remainingQuantity =
    reportedRemainingQuantity ??
    (ACTIVE_ORDER_STATES.has(state)
      ? Decimal.max(0, quantity.minus(filledQuantity))
      : TERMINAL_ORDER_STATES.has(state)
        ? new Decimal(0)
        : undefined);
  if (remainingQuantity?.gt(quantity) === true) {
    schemaFailure(
      `Order ${value.id} remaining quantity exceeds requested quantity`,
    );
  }
  if (
    ACTIVE_ORDER_STATES.has(state) &&
    (remainingQuantity === undefined ||
      !filledQuantity.plus(remainingQuantity).eq(quantity))
  ) {
    schemaFailure(
      `Order ${value.id} filled and remaining quantities contradict requested quantity`,
    );
  }
  ...
  const averageYesPrice = value.avgPx?.value;
  if (averageYesPrice !== undefined) {
    assertProbability(averageYesPrice, "order average fill YES price");
  }
  ...
  return {
    ...
    quantity,
    filledQuantity,
    ...optional("remainingQuantity", remainingQuantity),
    ...optional(
      "averageFillPrice",
      averageYesPrice === undefined
        ? undefined
        : yesPriceToCanonicalPrice(canonical.side, averageYesPrice),
    ),
    ...optional("fees", value.commissionNotionalTotalCollected?.value),
    ...optional(
      "executionPolicy",
      value.tif === undefined ? undefined : mapOrderExecutionPolicy(value.tif),
    ),
    ...optional("restUntil", value.goodTillTime),
    state,
...
export function mapCreateOrderResult(
  ...
  const remainingQuantity = Decimal.max(
    0,
    requestedQuantity.minus(filledQuantity),
  );
  ...
  if (
    allowResting &&
    ["NEW", "OPEN", "INFLIGHT", "PARTIALLY_FILLED"].includes(finalState) &&
    filledQuantity.lt(requestedQuantity)
  ) {
    return {
      status: "WORKING",
      orderId: value.id,
