...
export async function valuePortfolio(
  ...
  // A resting Polymarket US BUY moves cash out of buying power while it is
  // reserved. Add the exchange-reported open-order value back to account value,
  // but never to spendable capital.
  const reservedOpenOrderValue =
    exchange.id === "polymarket-us" ? snapshot.openOrderValue : new Decimal(0);
  const exchangeReportedValue = snapshot.buyingPower
    .plus(reservedOpenOrderValue)
    .plus(exchangePositionValue);
  ...
  const arenaAccountValue = snapshot.buyingPower
    .plus(reservedOpenOrderValue)
    .plus(liquidationValue);
