...
export interface ReconcilePortfolioTargetsInput {
  ...
  /** Deployment tolerance for nonzero targets; omitted means exact equality. */
  readonly targetRoundingToleranceUsd?: Decimal;
}
...
export function reconcilePortfolioTargets(
  ...
  const targetRoundingToleranceUsd =
    input.targetRoundingToleranceUsd ?? new Decimal(0);
  if (
    !targetRoundingToleranceUsd.isFinite() ||
    targetRoundingToleranceUsd.lt(0)
  ) {
    throw new RangeError(
      "targetRoundingToleranceUsd must be a finite non-negative decimal",
    );
  }
