...
const ExecutionReportSchema = z
  .object({
    ...
    executionPolicy: z.enum(["IOC", "GTD"]).optional(),
    restUntil: TimestampSchema.optional(),
    status: z.string().trim().min(1).max(100),
    filledQuantity: DecimalStringSchema,
    remainingQuantity: DecimalStringSchema.optional(),
...
const ExecutionAdvisorySchema = z
  .object({
    ...
    quantity: DecimalStringSchema,
    canonicalLimitPrice: DecimalStringSchema,
    executionPolicy: z.enum(["IOC", "GTD"]).default("IOC"),
    restUntil: TimestampSchema.optional(),
