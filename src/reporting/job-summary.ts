...
export function renderJobSummary(report: CycleReport): string {
  ...
  const lines = [
    ...
    "",
    `Execution attempts: **${currentCycleExecutions.length}**; fills: **${filled.length}**; fees: **$${fees.toFixed(4)}**; ambiguous: **${currentCycleExecutions.some((item) => item.status === "AMBIGUOUS") ? "yes" : "no"}**. These records contain only orders handled by this cycle.`,
    "",
  ...
  if (currentCycleExecutions.length > 0) {
    lines.push(
      "",
      "| Market | Side | Action | Status | Filled | Price | Fees |",
      "| --- | --- | --- | --- | ---: | ---: | ---: |",
      ...currentCycleExecutions.map(
        (execution) =>
          `| ${escapeCell(execution.marketSlug)} | ${execution.side} | ${execution.action} | ${execution.status} | ${execution.filledQuantity} | ${execution.averageFillPrice ?? "—"} | $${execution.fees} |`,
      ),
    );
  }
