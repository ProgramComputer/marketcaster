  ...
  if (
    (filledQuantity.gt(0) &&
      (averageFillPrice === undefined ||
        !averageFillPrice.gt(0) ||
        !averageFillPrice.lt(1))) ||
...
function submissionResult(
  ...
  return inspectDurableResult(envelope.data.result, correlation);
}
...
function reconciliationResult(
  ...
  return inspectDurableResult(envelope.data.finalResult, correlation);
}
    ...
    if (durableResult !== "TERMINAL") {
      issues.push(
        issue(
          ...
          manifest.cycleId,
          journalDirectory,
          `Order ${attemptId} intent has no demonstrably terminal durable outcome`,
          referencePaths,
        ),
      );
      continue;
