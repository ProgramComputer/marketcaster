...
function accountReport(
  ...
  return {
    ...
    openOrderValue: snapshot.openOrderValue.toFixed(),
    ...
    allocation: {
      ...
      reservedOrderValue: snapshot.openOrderValue.toFixed(),
      ...
      ...(hasPositiveAccountValue
        ? {
            cashFraction: snapshot.buyingPower
              ...
              .toFixed(),
            reservedOrderFraction: snapshot.openOrderValue
              .div(valuation.arenaAccountValue)
              .toFixed(),
    ...
    positions: snapshot.positions.map((position) => {
      ...
      return {
        ...
        lifecycleState:
          position.lifecycleState ?? (position.expired ? "EXPIRED" : "UNKNOWN"),
        payoutState: position.payoutState ?? "UNKNOWN",
        ...(position.positionLifecycleId === undefined
          ? {}
...
function observedTradeReport(trade: TradeActivity): ObservedTradeReport {
  return {
    ...
    ...(trade.orderId === undefined ? {} : { orderId: trade.orderId }),
    ...(trade.fillId === undefined ? {} : { fillId: trade.fillId }),
    ...(trade.positionLifecycleId === undefined
      ? {}
      : { positionLifecycleId: trade.positionLifecycleId }),
...
function observedSettlementReport(
  ...
  return {
    ...
    payoutState: settlement.payoutState ?? "UNKNOWN",
    ...(settlement.positionLifecycleId === undefined
      ? {}
      : { positionLifecycleId: settlement.positionLifecycleId }),
    ...(settlement.payoutAmount === undefined
      ? {}
      : { payoutAmount: settlement.payoutAmount.toFixed() }),
...
function activityView(
  ...
  return {
    ...
    realizedPnlTradeCount: closedTrades.length,
    ...
    realizedPnlTrades: closedTrades.map(observedTradeReport),
...
function activityIdentity(activity: AccountActivity): string {
  ...
  if (activity.kind === "RESOLUTION") {
    return [
      ...
      activity.realizedPnl.toFixed(),
      activity.positionLifecycleId ?? "",
      activity.payoutState ?? "UNKNOWN",
      activity.payoutAmount?.toFixed() ?? "",
      activity.paidAt?.toISOString() ?? "",
    ].join("\u0000");
  }
  return [
    activity.kind,
...
function exchangeObservedActivity(
  ...
  const trades = latestTradeStates(after.recentActivities).filter(
    (trade) => trade.state !== "TRADE_STATE_BUSTED",
  );
  const exactOrderMatches = executions.flatMap((execution) => {
    if (
      execution.intentId === undefined ||
      execution.orderId === undefined ||
      executions.filter((candidate) => candidate.orderId === execution.orderId)
        .length !== 1
    )
      return [];
    const matched = trades.filter(
      (trade) =>
        ...
        trade.marketSlug === execution.marketSlug &&
        (trade.side === undefined || trade.side === execution.side) &&
        (trade.action === undefined || trade.action === execution.action),
    );
    ...
    return [
      {
        ...
        tradeIds: matched.map((trade) => trade.tradeId),
        fillIds: matched.flatMap((trade) =>
          trade.fillId == null ? [] : [trade.fillId],
        ),
      },
    ];
  });
...
function agentStateSnapshotReport(
  ...
  return {
    ...
    ...(state.mode === "PERSISTENT" && state.inactiveBeliefCount !== undefined
      ? { inactiveBeliefCount: state.inactiveBeliefCount }
      : {}),
...
function executionReports(execution: ExecutionRun): readonly ExecutionReport[] {
  return execution.attempts.map((attempt) => {
    ...
    return {
      ...(attempt.intentId === undefined ? {} : { intentId: attempt.intentId }),
      ...(attempt.failure === undefined ? {} : { failure: attempt.failure }),
      ...(attempt.cooldown === undefined ? {} : { cooldown: attempt.cooldown }),
      ...
      executionPolicy: attempt.validated.order.executionPolicy ?? "IOC",
      ...(attempt.validated.order.restUntil === undefined
        ? {}
        : { restUntil: attempt.validated.order.restUntil.toISOString() }),
      ...
      ...(result?.remainingQuantity === undefined
        ? {}
        : { remainingQuantity: result.remainingQuantity.toFixed() }),
...
function cycleOutcome(
  ...
  if (executions.some((execution) => execution.status === "WORKING")) {
...
export function buildCycleReport(input: BuildCycleReportInput): CycleReport {
  ...
  const status: CycleReport["status"] =
    outcome === "FILLED" || outcome === "ORDER_WORKING"
      ? "SUCCESS"
      : outcome === "FAILED"
        ? "FAILED"
        : outcome === "AMBIGUOUS"
  ...
  return {
    ...
    risk: {
      accepted: input.validation.accepted.map((accepted) => ({
        ...
        executionPolicy: accepted.order.executionPolicy ?? "IOC",
        ...(accepted.order.restUntil === undefined
          ? {}
          : { restUntil: accepted.order.restUntil.toISOString() }),
