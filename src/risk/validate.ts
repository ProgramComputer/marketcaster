...
export interface ManagedRestingBuyOrdersPolicy {
  readonly enabled: boolean;
  readonly maximumLifetimeMinutes: number;
}

export interface ValidateProposalsInput {
  readonly minimumNearTouchBuyNotionalUsd?: Decimal | undefined;
  readonly depthPriceBand?: Decimal | undefined;
...
function fitBuyQuantity(
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    if (
      executionPolicy === "IOC"
        ? depth.fillableQuantity.lt(market.minimumTradeQuantity)
        : depth.fillableQuantity.lte(0)
    ) {
      reject(
        "NO_DEPTH",
        executionPolicy === "IOC"
          ? "Executable depth is below the market minimum quantity"
          : "A managed resting order requires some immediately executable depth",
      );
    }
    if (executionPolicy === "IOC" && !depth.fullyFillable) {
    ...
    // A resting order can fill later anywhere through its limit, so its fees,
    // edge, Kelly budget, and cash reservation must use that worst-case price.
    const sizingPrice = executionPolicy === "GTD" ? limitPrice : depth.vwap;
    const fees = estimateExchangeTakerFee(exchangeId, quantity, sizingPrice);
...
async function assessOne(
  ...
  if (
    proposal.action === "BUY" &&
    input.minimumNearTouchBuyNotionalUsd !== undefined
  ) {
    const minimum = input.minimumNearTouchBuyNotionalUsd;
    if (!minimum.isFinite() || minimum.lt(0))
      reject(
        "INVALID_PROPOSAL",
        "Minimum buy notional must be finite and non-negative",
      );
    const available = nearTouchBuyNotional(
      book,
      proposal.side,
      input.depthPriceBand ?? new Decimal(0),
      authorizedLimitPrice,
    );
    if (available.lt(minimum))
      reject(
        "NO_DEPTH",
        "Executable near-touch BUY principal is below the configured minimum",
      );
  }
  const useManagedRestingBuyOrder =
    proposal.action === "BUY" &&
    input.exchange.id === "polymarket-us" &&
    input.managedRestingBuyOrders?.enabled === true;
  if (useManagedRestingBuyOrder) {
    const lifetimeMinutes =
      input.managedRestingBuyOrders.maximumLifetimeMinutes;
    if (
      ...
      reject(
        "INVALID_MARKET_CONSTRAINTS",
        "Managed resting-order lifetime must be an integer from 1 through 15 minutes",
      );
    }
  }
  ...
  // Give entry orders two ticks of quote-movement tolerance. The model or
  // automatic positive-edge limit remains the hard ceiling.
  const limitPrice = Decimal.min(
    authorizedLimitPrice,
    quote.ask.plus(market.priceTick.mul(2)),
  );
  const executionPolicy = useManagedRestingBuyOrder ? "GTD" : "IOC";
  let restUntil: Date | undefined;
  if (executionPolicy === "GTD") {
    const configuredRestUntil =
      validatedAt.getTime() +
      (input.managedRestingBuyOrders?.maximumLifetimeMinutes ?? 0) * 60_000;
    const closesAt = market.closesAt?.getTime();
    if (closesAt !== undefined && !Number.isFinite(closesAt)) {
      reject("INVALID_MARKET_CONSTRAINTS", "Market close time is invalid");
    }
    const marketBoundedRestUntil =
      closesAt === undefined
        ? configuredRestUntil
        : Math.min(
            configuredRestUntil,
            closesAt - MARKET_CLOSE_RESTING_SAFETY_MILLISECONDS,
          );
    if (
      marketBoundedRestUntil - validatedAt.getTime() <
      MINIMUM_USEFUL_RESTING_WINDOW_MILLISECONDS
    ) {
      reject(
        "MARKET_CLOSED",
        "Market closes too soon for a safely bounded resting-order window",
      );
    }
    restUntil = new Date(marketBoundedRestUntil);
  }
  const minimumOrder: ImmediateOrder = {
    ...
    ...(executionPolicy === "GTD" && restUntil !== undefined
      ? { executionPolicy, restUntil }
      : {}),
  };
  ...
  const netEdge = calculateNetEdge(
    authorizationProbability,
    executionPolicy === "GTD" ? limitPrice : maximumSized.depth.vwap,
    feePerContract,
  );
  if (netEdge.lte(0)) {
    reject(
...
function finalizeBuy(
  ...
  return {
    ...
    order: {
      ...
      quantity: sized.quantity,
      ...(candidate.executionPolicy === "GTD" &&
      candidate.restUntil !== undefined
        ? {
            executionPolicy: candidate.executionPolicy,
            restUntil: candidate.restUntil,
          }
        : {}),
    },
    depth: sized.depth,
    ...
    expectedSpend:
      candidate.executionPolicy === "GTD"
        ? sized.quantity.mul(candidate.limitPrice).plus(sized.fees)
        : sized.depth.principal.plus(sized.fees),
...
/** A refresh may widen uncertainty; it cannot turn a derived point into certainty. */
export function applyFreshForecastProbability(
  proposal: RiskProposal,
  freshProbability: Decimal,
): RiskProposal {
  if (
    !Decimal.isDecimal(freshProbability) ||
    !freshProbability.isFinite() ||
    freshProbability.lt(0) ||
    freshProbability.gt(1)
  ) {
    throw new RangeError(
      "Fresh forecast must be a finite probability from zero through one",
    );
  }
  return {
    ...proposal,
    estimatedProbability: freshProbability,
    probabilityLowerBound: Decimal.min(
      proposal.probabilityLowerBound,
      freshProbability,
    ),
    probabilityUpperBound: Decimal.max(
      proposal.probabilityUpperBound,
      freshProbability,
    ),
  };
...
export async function validateProposals(
  ...
  for (const {
    ...
    if (
      ...
      indexedRejections.push({
        ...
        rejection: {
          ...
          code: "MARKET_CHANGED",
          reason:
            "A required fresh policy forecast was unavailable or invalid at validation; the submitted probability cannot authorize execution",
        },
      });
      continue;
    }
    const proposal =
      freshProbability === undefined
        ? submittedProposal
        : applyFreshForecastProbability(submittedProposal, freshProbability);
    try {
