...
export interface DecisionResearchToolsOptions {
  ...
  /** Passed-event signals that require inspection and source-linked follow-through. */
  readonly requiredPriorityEvidenceMarketSlugs?: readonly string[];
...
// One attributable first-party/resolver source is enough to force an honest
// review of a passed-event signal. Entry authorization separately enforces the
// configured current-source policy, so this readiness gate must not require an
// unrelated secondary domain and consume the evidence-read budget.
...
function marketVersusFindHint(title: string): string {
  const normalized = title.replace(/\s+/gu, " ").trim();
  const match = /(.+?)\s+vs\.?\s+(.+?)(?:\s+scheduled\b|\?|$)/iu.exec(
    normalized,
  );
  if (match === null) return normalized;
  const left = (match[1] ?? "").replace(/^.*\b(?:event|match)\s+/iu, "").trim();
...
function tableTennisSetComplete(
  leftPoints: number,
  rightPoints: number,
): boolean {
  return (
...
function tableTennisSetWinProbability(
  leftPoints: number,
  rightPoints: number,
...
function firstToThreeMatchWinProbability(
  leftSets: number,
  rightSets: number,
...
function neutralTableTennisScoreReference(line: string): string | undefined {
  const state =
    /\|\s*([^|]+?)\s+(\d+)\s+vs\s+([^|]+?)\s+(\d+)\s*\|\s*statusId\s+(\d+)/iu.exec(
      line,
    );
  if (state?.[5] !== "2") return undefined;

  const reportedLeftSets = Number.parseInt(state[2] ?? "0", 10);
  const reportedRightSets = Number.parseInt(state[4] ?? "0", 10);
  const setScores = [...line.matchAll(/set\s+\d+\s+(\d+)-(\d+)/giu)].map(
    (match) => ({
      left: Number.parseInt(match[1] ?? "0", 10),
      right: Number.parseInt(match[2] ?? "0", 10),
    }),
  );
  let scoreLeftSets = 0;
  let scoreRightSets = 0;
  let currentSet: { readonly left: number; readonly right: number } | undefined;
  for (const score of setScores) {
    if (tableTennisSetComplete(score.left, score.right)) {
      if (score.left > score.right) scoreLeftSets += 1;
      else scoreRightSets += 1;
    } else {
      currentSet = score;
    }
  }

  const scoreSetCount = scoreLeftSets + scoreRightSets;
  const reportedSetCount = reportedLeftSets + reportedRightSets;
  const leftSets =
    scoreSetCount >= reportedSetCount ? scoreLeftSets : reportedLeftSets;
  const rightSets =
    scoreSetCount >= reportedSetCount ? scoreRightSets : reportedRightSets;
  const leftProbability =
    currentSet === undefined
      ? firstToThreeMatchWinProbability(leftSets, rightSets)
      : tableTennisSetWinProbability(currentSet.left, currentSet.right) *
          firstToThreeMatchWinProbability(leftSets + 1, rightSets) +
        (1 - tableTennisSetWinProbability(currentSet.left, currentSet.right)) *
          firstToThreeMatchWinProbability(leftSets, rightSets + 1);
  const boundedLeftProbability = Math.min(1, Math.max(0, leftProbability));
  return `neutral score-only reference (future points 50/50; not a forecast): ${(state[1] ?? "left player").trim()} ${(boundedLeftProbability * 100).toFixed(1)}%, ${(state[3] ?? "right player").trim()} ${((1 - boundedLeftProbability) * 100).toFixed(1)}%`;
}

function inverseMonotonicProbability(
...
function participantMatchesLiveName(
  participant: string,
  liveName: string,
): boolean {
  const terms = evidenceQueryTerms(participant);
  const normalizedName = liveName.toLocaleLowerCase("en-US");
  return (
    terms.length > 0 && terms.every((term) => normalizedName.includes(term))
  );
}

function openingPriceAdjustedTableTennisReference(
  line: string,
  marketParticipants: readonly string[],
  sessionOpenYesPrice: string | undefined,
): string | undefined {
  if (sessionOpenYesPrice === undefined || marketParticipants.length !== 2) {
    return undefined;
  }
  const state =
    /\|\s*([^|]+?)\s+(\d+)\s+vs\s+([^|]+?)\s+(\d+)\s*\|\s*statusId\s+(\d+)/iu.exec(
  ...
  let marketYesProbability: number;
  try {
    const parsed = new Decimal(sessionOpenYesPrice);
    if (!parsed.isFinite() || parsed.lt(0) || parsed.gt(1)) return undefined;
    marketYesProbability = parsed.toNumber();
  } catch {
    return undefined;
  }
  const leftName = (state[1] ?? "left player").trim();
  const rightName = (state[3] ?? "right player").trim();
  const marketFirst = marketParticipants[0] ?? "";
  const marketSecond = marketParticipants[1] ?? "";
  const openingLeftMatchProbability =
    participantMatchesLiveName(marketFirst, leftName) &&
    participantMatchesLiveName(marketSecond, rightName)
      ? marketYesProbability
      : participantMatchesLiveName(marketFirst, rightName) &&
          participantMatchesLiveName(marketSecond, leftName)
        ? 1 - marketYesProbability
        : undefined;
  if (openingLeftMatchProbability === undefined) return undefined;

  const leftSetWinProbability = inverseMonotonicProbability(
    openingLeftMatchProbability,
    (probability) => firstToThreeMatchWinProbability(0, 0, probability),
  );
  const leftPointWinProbability = inverseMonotonicProbability(
    leftSetWinProbability,
    (probability) => tableTennisSetWinProbability(0, 0, probability),
  );
  const reportedLeftSets = Number.parseInt(state[2] ?? "0", 10);
  const reportedRightSets = Number.parseInt(state[4] ?? "0", 10);
  const setScores = [...line.matchAll(/set\s+\d+\s+(\d+)-(\d+)/giu)].map(
  ...
  const boundedLeftProbability = Math.min(1, Math.max(0, leftProbability));
  return `opening-price-updated reference (YES open ${(marketYesProbability * 100).toFixed(1)}%; stationary strength and score only; not a forecast): ${leftName} ${(boundedLeftProbability * 100).toFixed(1)}%, ${rightName} ${((1 - boundedLeftProbability) * 100).toFixed(1)}%`;
}

export function liveEvidenceLinePreview(
  rawText: string,
  findHint: string,
  sessionOpenYesPrice?: string,
): LiveEvidenceLinePreview | undefined {
  const participants = findHint.split(/\s+vs\s+/iu);
  if (participants.length !== 2) return undefined;
  const leftTerms = evidenceQueryTerms(participants[0] ?? "");
  const rightTerms = evidenceQueryTerms(participants[1] ?? "");
  if (leftTerms.length === 0 || rightTerms.length === 0) return undefined;
  const candidates = sanitizeExternalText(rawText)
    .split(/\r?\n/gu)
    .map((line) => line.trim())
    .filter((line) => line.length > 0)
    .flatMap((line) => {
      const normalized = line.toLocaleLowerCase("en-US");
      const leftMatches = leftTerms.filter((term) => normalized.includes(term));
      const rightMatches = rightTerms.filter((term) =>
        normalized.includes(term),
      );
      return leftMatches.length === 0 || rightMatches.length === 0
        ? []
        : [{ line, score: leftMatches.length + rightMatches.length }];
    })
    .toSorted((left, right) => right.score - left.score)
    .slice(0, 3)
    .map(({ line }) => line);
  if (candidates.length === 0) return undefined;
  const evidenceLines = [...new Set(candidates)];
  const previewLines = evidenceLines.flatMap((line) => {
    const references = [
      neutralTableTennisScoreReference(line),
      openingPriceAdjustedTableTennisReference(
        line,
        participants,
        sessionOpenYesPrice,
      ),
...
export function systemLiveEvidenceSources(
  details: DetailedMarketContext,
  now = new Date(),
  ...
  const context = [
    ...
    details.title,
    details.description,
    details.settlementRules,
  ]
    .join(" ")
    .toLowerCase();
  const isCourtTennis =
    context.includes("tennis") || /\b(?:atp|wta|itf)\b/iu.test(context);
  if (!context.includes("sport") && !isCourtTennis) return [];

  if (context.includes("setka cup")) {
    return [
      {
        title: "Official Setka Cup live and recent-results feed",
        url: "https://tabletennis.setkacup.com/api/Matches/widget/en",
        findHint: marketVersusFindHint(details.title),
      },
    ];
  }
  if (!isCourtTennis || context.includes("table tennis")) return [];

  const date = now.toISOString().slice(0, 10).replaceAll("-", "");
  const tours =
    /\batp\b/iu.exec(context) !== null
      ? ["atp"]
      : /\bwta\b/iu.exec(context) !== null
        ? ["wta"]
        : ["atp", "wta"];
  return tours.map((tour) => ({
    title: `ESPN ${tour.toUpperCase()} tennis live scoreboard`,
    url: `https://site.web.api.espn.com/apis/site/v2/sports/tennis/${tour}/scoreboard?dates=${date}`,
    findHint: marketVersusFindHint(details.title),
  }));
}
...
export class DecisionResearchSession {
  ...
  private async registerSystemLiveEvidenceSources(
    ...
    const sources = systemLiveEvidenceSources(details);
    ...
    return Promise.all(
      sources.map(async (source): Promise<SystemLiveEvidenceSource> => {
        ...
        try {
          ...
          const linePreview = liveEvidenceLinePreview(
            page.text,
            source.findHint,
  ...
  private priorityResearchUnmet(): readonly string[] {
    ...
    return [
      ...(missingInspections.length === 0
        ? []
        : [
            `inspect passed-event priority market(s): ${missingInspections.join(", ")}`,
          ]),
      ...(missingEvidence.length === 0
        ? []
        : [
            `bind observed current sources from ${MINIMUM_PRIORITY_EVIDENCE_ATTEMPT_DOMAINS} distinct domains with read_evidence_source for quoted passed-event market(s): ${missingEvidence.join(", ")}`,
          ]),
    ];
  }
  ...
  public async execute(
    ...
    switch (name) {
      ...
      case "submit_trade_plan": {
        ...
        // Evidence for a passed priority candidate must not delay a separately
        // grounded, non-held exposure target.  The target still passes through
        // ordinary evidence, coverage, reconciliation, and risk validation.
        // Keep the priority-research gate for cycles that would add no new
        // market exposure, where it prevents a premature all-pass decision.
        const addsNewMarketExposure =
          decision.portfolioTargets.some(
            (target) =>
              target.targetCostBasisFraction.gt(0) &&
              this.detailsBySlug.get(target.marketSlug)?.held !== true,
          ) ||
          decision.proposals.some(
            (proposal) =>
              proposal.action === "BUY" &&
              this.detailsBySlug.get(proposal.marketSlug)?.held !== true,
          );
        const priorityResearchUnmet = this.priorityResearchUnmet();
        if (!addsNewMarketExposure && priorityResearchUnmet.length > 0) {
          return safeToolError(
