...
import {
  ...
  type OpportunityPrioritySignal,
...
export interface MarketSelectionArmSummary {
  ...
  readonly resolverWindowCount: number;
  readonly measurementFamilyCount: number;
  readonly passedEventCount: number;
  readonly nearSettlementCount: number;
...
export interface MarketSelectionExperimentReport {
  ...
  readonly experimentId: "high-leverage-market-selection-v2";
...
function prioritySignalKey(signal: OpportunityPrioritySignal): string {
  if (signal.kind !== "RESOLVER_WINDOW_CANDIDATE") return signal.kind;
  return `${signal.kind}:${signal.resolverPattern}`;
}
...
function summarize(
  ...
  let resolverWindowCount = 0;
  let measurementFamilyCount = 0;
  let passedEventCount = 0;
  let nearSettlementCount = 0;
  for (const item of selections) {
    ...
    const key = prioritySignalKey(signal);
    prioritySignalCounts[key] = (prioritySignalCounts[key] ?? 0) + 1;
    if (signal.kind === "RESOLVER_WINDOW_CANDIDATE") resolverWindowCount += 1;
    if (signal.kind === "SHORT_HORIZON_MEASUREMENT_FAMILY") {
      measurementFamilyCount += 1;
    }
    if (
      signal.kind === "EVENT_DATE_BEFORE_MARKET_CLOSE" &&
      signal.eventTiming === "PASSED"
    ) {
      passedEventCount += 1;
    }
    if (signal.kind === "NEAR_SETTLEMENT_DECISIVE_PRICE") {
      nearSettlementCount += 1;
    }
  }
  return {
...
function arm(
  ...
  const selections = buildOpportunityBoard(catalog, policy, frozenAt, variant);
  return {
    variant,
...
export function replayMarketSelectionExperiment(
  ...
  const control = arm("GENERALIST_CONTROL", catalog, policy, frozenAt);
  const treatment = arm("RESOLVER_LAG_TREATMENT", catalog, policy, frozenAt);
  ...
  return {
    ...
    experimentId: "high-leverage-market-selection-v2",
    hypothesis:
      "A bounded resolver-lag and short-horizon measurement-family lane surfaces more mechanically verifiable opportunities than a generalist exchange-ranked board without relying on transaction frequency.",
    frozenAt: frozenAt.toISOString(),
    ...
    limitations: [
      "This is a paired market-selection comparison, not a realized-return backtest.",
      "Only the treatment board is forwarded to the decision model in the live cycle, so downstream control quality is not yet observed.",
      "Priority signals are discovery hints; they do not establish settlement, evidence validity, executable edge, or profitability.",
    ],
  };
}
...
export async function replayMarketSelectionArtifact(
  ...
  return replayMarketSelectionExperiment(parsed.data.data);
}
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  if (artifactPath === undefined) {
    process.stderr.write(
      "Usage: node dist/src/experiments/market-selection.js <market-selection-snapshot.json>\n",
    );
    return 2;
  }
  try {
    const report = await replayMarketSelectionArtifact(artifactPath);
