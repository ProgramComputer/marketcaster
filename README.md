...
exchange catalog and allocates no BUY capital. A deployment supplies its own
selection, forecast, and allocation policies through a versioned module.
...
The repository includes neutral reference configuration, prompt contracts, and
synthetic checks. Deployment policies, source recipes, experiment definitions,
learned lessons, and historical account results belong in deployment repositories.
A trusted policy module receives a versioned engine API; it cannot replace the
engine's risk assessment or submit orders through that API.
...
enrichment mechanisms accept deployment callbacks; production rankings and
source-selection recipes are not built into the engine. Reference configuration
values are illustrative safety and resource limits, not calibrated parameters.
...
MARKETCASTER_STRATEGY_PATH=/deployment/strategy/policy.mjs \
...
The trading job runs only for private caller repositories, in either mode.
Public manual dispatches skip the trading job. Local observe runs remain
available. CI validates pushes; its pull-request trigger is disabled.
...
jobs:
  cycle:
    uses: ProgramComputer/marketcaster/.github/workflows/marketcaster-cycle.yml@FULL_TESTED_COMMIT_SHA
    with:
      mode: ${{ github.event_name == 'schedule' && 'live' || inputs.mode }}
      engine_repository: ProgramComputer/marketcaster
      engine_ref: FULL_TESTED_COMMIT_SHA
      strategy_path: strategy/policy.mjs
...
Replace both `FULL_TESTED_COMMIT_SHA` placeholders with the same verified engine
commit. When a private caller adds a schedule, the mode expression preserves the
