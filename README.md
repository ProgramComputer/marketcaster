...
```sh
MARKETCASTER_CONFIG_PATH=/private/config/production.json \
MARKETCASTER_DECISION_PROMPT_PATH=/private/strategy/decision-system.md \
MARKETCASTER_REPORT_DIR=/private/reports \
node --env-file=.env dist/src/index.js
```
...
A private deployment can call the workflow with its own schedule and private
paths:
...
This expression preserves the intended semantics: scheduled production runs are
live, while manual runs default to observe and become live only when selected.
Artifacts and caches belong to the caller repository, so a private caller keeps
...
```text
marketcaster-production/
  .github/workflows/production-cycle.yml
